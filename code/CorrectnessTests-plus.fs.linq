...
let naive_a (s:string) (z:string) : int[,]*char[,] =
    let s, z = " " + s, " " + z 
    let m, n = s.Length, z.Length
    let C = Array2D.create (m) (n) 0
    let P = Array2D.create (m) (n) ' '
    for i = 1 to m-1 do P.[i,0] <- s.[i]
    for j = 1 to n-1 do P.[0,j] <- z.[j]
